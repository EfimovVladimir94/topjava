...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.ALL_BETWEEN, query = "SELECT m FROM Meal m WHERE m.dateTime BETWEEN :startdate AND :enddate AND m.user.id=:userid ORDER BY m.dateTime DESC")
})
